...
void empilhar(int tamanho, int i);
...
void criarTabelaHash(int tamTabelaHash){
    int aux;
    tabelaHash = malloc(sizeof(elemento) * tamTabelaHash);
...
void empilhar(int tamanho, int i){
    ...
    if (inicio < tamanho && tabelaHash[inicio].chave > tabelaHash[maior].chave)
        maior = inicio;


    if (fim < tamanho && tabelaHash[fim].chave > tabelaHash[maior].chave)
        maior = fim;


    if (maior != i) {
        temp = tabelaHash[i];
        tabelaHash[i] = tabelaHash[maior];
        tabelaHash[maior] = temp;


        empilhar(tamanho, maior);
    }
}
